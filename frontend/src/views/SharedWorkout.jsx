...
export default function SharedWorkout({ shareId }) {
  ...
  return <div className="shared-workout-page">
    ...
    <div className="shared-workout-day-card">{plan.days.map(day => <button key={day.id} className="shared-workout-day-card" onClick={() => setSelectedDayId(day.id)}><span><strong>{day.name}</strong><small>{day.exercises.length} esercizi</small></span></button>)}</div>
    <p className="shared-workout-note">Puoi allenarti senza account. I risultati non vengono salvati.</p>
  </div>;
}
